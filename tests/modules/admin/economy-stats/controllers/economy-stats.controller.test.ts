...
const PATH = '/api/admin/economy-stats';

describe('registerAdminEconomyStatsModuleRoutes', () => {
  ...
  beforeEach(() => {
    ...
    prismaMock = {
      prisma: {
        ...
        mining_coins: { findMany: vi.fn().mockResolvedValue([]) },
  ...
  async function loadGuardedApp() {
    ...
    const { registerAdminEconomyStatsModuleRoutes } = await import(
      '../../../../../server/modules/admin/economy-stats/controllers/economy-stats.controller.js'
    );
    const app = fakeApp();
    registerAdminEconomyStatsModuleRoutes(app as any, { isAdmin: createIsAdminMiddleware({ parseCookies }) });
    return app;
  }
  ...
  it('sem sessão → 401', async () => {
    ...
    expect(prismaMock.prisma.mining_coins.findMany).not.toHaveBeenCalled();
  });
  ...
  it('admin com tab reports → 200 array', async () => {
    prismaMock.prisma.users.findUnique.mockResolvedValue({
      is_admin: 1,
      is_super_admin: 0,
  ...
  it('query extra é ignorada (caller não envia params; sem 400)', async () => {
    prismaMock.prisma.users.findUnique.mockResolvedValue({ is_admin: 1, is_super_admin: 1, admin_permissions: null });
    const app = await loadGuardedApp();
    const res = fakeRes();
    await dispatch(
      app,
      `GET ${PATH}`,
      { headers: {}, userId: 1, url: PATH, originalUrl: PATH, method: 'GET', query: { period: '1;drop' } },
      res
    );
    expect(res.statusCode).toBe(200);
  });
