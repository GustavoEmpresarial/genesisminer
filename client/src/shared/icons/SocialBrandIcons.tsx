...
const socialBtnBase =
  'inline-flex items-center justify-center rounded-lg border border-transparent p-2 transition focus-visible:outline-none focus-visible:ring-2';
...
export function DiscordCommunityLink({
  ...
  className = '',
  onClick
}: IconProps & { className?: string; onClick?: () => void }) {
  return (
    <a
      ...
      onClick={onClick}
      className={`${socialBtnBase} text-slate-400 hover:border-[#5865F2]/35 hover:bg-[#5865F2]/12 hover:text-[#7289da] focus-visible:ring-[#5865F2]/50 ${className}`}
      title="Discord — Genesis Miner"
...
export function TelegramCommunityLink({
  ...
  return (
    <a
      ...
      rel="noopener noreferrer"
      onClick={onClick}
      className={`${socialBtnBase} text-slate-400 hover:border-[#229ED9]/35 hover:bg-[#229ED9]/12 hover:text-[#26A5E4] focus-visible:ring-[#229ED9]/50 ${className}`}
      title="Telegram — Genesis Miner"
      aria-label="Abrir Telegram da comunidade"
    >
